...
function getOverallStatus(instances) {
    if (instances.length === 0)
        return 'healthy';
    const statuses = instances.map((i) => i.status);
    if (statuses.every((s) => s === 'offline'))
        return 'offline';
    if (statuses.some((s) => s === 'offline' || s === 'degraded'))
        return 'degraded';
    return 'healthy';
}
function renderDashboard(metrics) {
    ...
    const authModeDisplay = metrics.server.mode === 'oauth'
        ? 'OAuth 2.1'
...
<body>
  <div class="dashboard">
    ...
    <div class="header-meta">
      Uptime: ${(0, metrics_js_1.formatUptime)(metrics.server.uptime)} | Mode: ${authModeDisplay} | Sessions: ${metrics.sessions.total}
    </div>
    <div class="content">
      <div class="section">
